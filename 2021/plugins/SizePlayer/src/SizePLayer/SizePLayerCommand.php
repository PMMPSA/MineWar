        ...
        if($player->hasPermission("size.command")) {
            if(isset($args[0])) {
                if(is_numeric($args[0])) {
                    if($args[0] > 5) {
                      $player->sendMessage(TF::RED. "Không Thể chỉnh size quá 5");
                      return true;
                    }elseif($args[0] <= 0) {
                      $player->sendMessage(TF::RED. "không thẻ chỉnh size dưới 0");
                      return true;
                    }
                    $this->plugin->size[$player->getName()] = $args[0];
                    $player->setScale((float)$args[0]);
                    $player->sendMessage("§6§lBạn Đã Chỉnh Kích Thước Thành:".TF::GOLD . $args[0]."§a!");
                }elseif($args[0] == "reset") {
